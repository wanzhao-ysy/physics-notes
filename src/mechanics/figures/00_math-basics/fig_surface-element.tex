...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[x=1cm, y=1cm, z=-0.6cm, scale=0.6]
        ...
        \begin{scope}[shift={(3.8, 3.2)}, rotate=-10]
            ...
            % 面元标注 dS
            \node[below right, font=\footnotesize] at (p2) {$\d \symbfit{S}$};
            ...
            % 法向矢量 n (稍长)
            \draw[->, thick, blue] (center) -- ++(70:1.5) node[above] {$\symbfit{n}$};

            % 面元矢量 dS vector (稍短，方向相同)
            \draw[->, thick, red] (center) -- ++(70:0.8) node[right, font=\footnotesize, xshift=2pt] {$\d \symbfit{S} = \d S \symbfit{n}$};
        \end{scope}
